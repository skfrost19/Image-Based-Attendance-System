...
def verify_faces_concurrently(test_faces: list, db_faces: list) -> dict:
    ...
    try:
        ...
        with multiprocessing.Pool() as pool:
            face_pairs = [
                (test_face, db_face) for test_face in test_faces for db_face in db_faces
            ]
            results = pool.map(verify_face_pair, face_pairs)

        # Combine test_faces, db_faces, and results into a dictionary {db_face}
        verified_faces = {
            db_face.split("\\")[-1].split(".")[0]: result
            for ((test_face, db_face), result) in zip(face_pairs, results)
        }
